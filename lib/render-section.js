...
const section = sectionData
  ...
  .reduce((list, item) => `${list}\n<li>${item}</li>`, `## ${whichOnes.replace(/^\w/, c => c.toUpperCase())}\n\n<details ${isOpen ? 'open' : ''}><summary>collapse/expand</summary>${isOrdered ? `<ol start="${oLstart ? oLstart : '1'}">` : '<ul  style="list-style-type:none;">'}\n`)
